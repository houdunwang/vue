  ...
  <main v-if="user">
    ...
    <section class="2xl:w-page mx-auto relative z-10">
      <nav class="flex justify-center flex-wrap">
        <router-link
          :to="{ name: 'topic.space', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string)=='topic.space'}">
          TA的贴子
        </router-link>
        <router-link
          :to="{ name: 'sign.space', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string).includes('sign') }">
          历史签到
        </router-link>
        <router-link
          :to="{ name: 'video.space', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string) =='video.space' }">
          最近学习
        </router-link>
        <router-link
          :to="{ name: 'lesson.space.favorite', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string) == 'lesson.space.favorite' }">
          课程收藏
        </router-link>
        <!-- <router-link
          :to="{ name: 'question.space', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string) == 'question.space' }">
          TA出的题
        </router-link> -->
        <router-link
          :to="{ name: 'video.space.favorite', params: { uid: $route.params?.uid } }"
          :class="{ active: ($route.name as string) =='video.space.favorite' }">
          视频收藏
        </router-link>
        <router-link
          :to="{ name: 'topic.space.favorite', params: { uid: $route.params?.uid } }"
