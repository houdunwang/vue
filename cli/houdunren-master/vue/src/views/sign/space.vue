...
await Promise.all([getUserSignList(+route.params.uid, route.query.page || 1)])
</script>
...
<template>
  <main v-if="userCollection">
    ...
    <div class="bg-white border p-2 rounded-md mb-2 md:hidden" v-for="item of userCollection.data" :key="item.id">
      <div class="flex flex-col justify-between">
        ...
        <div class="text-sm flex gap-3 font-light mt-1">
          <router-link :to="{ name: 'sign.space', params: { uid: item.user_id } }" class="opacity-70">
            {{ item.user.name }}
          </router-link>
    ...
    <Pagination
      ...
      @current-change="
        $router.push({ name: 'sign.space', params: { uid: $route.params.uid }, query: { page: $event } })
      " />
  </main>
</template>
