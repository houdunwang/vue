    ...
    <section class="bg py-5 xl:py-8 text-white relative overflow-hidden">
      <div class="2xl:w-page mx-auto p-2">
        ...
        <div class="flex items-center gap-2">
          <DownloadLesson :download_address="model?.system?.download_address" size="large" />
          <Favorite size="large" type="Lesson" :mid="model.id" />
          <router-link :to="{ name: 'lesson.edit', params: { id: model.id } }" v-if="isAdministrator()">
            <el-button type="info" size="large"> 编辑 </el-button>
          </router-link>
        </div>
      </div>
