...
const { video, videos } = useSearch()
video(route.query.content, route.query.page || 1)
</script>
...
<template>
  <main v-if="videos">
    <HdCard>
      ...
      <VideoItem v-for="video of videos.data" :key="video.id" :video="video" class="p-5 py-5" />
      <div class="p-5 flex items-center justify-center gap-1 text-gray-600 text-sm" v-if="videos.data.length == 0">
