...
<template>
  <Layout>
    ...
    <section v-if="!collection?.data.length" class="p-3 bg-white rounded-lg text-center">
      所有题都被审核过了 <br />
      <el-button type="primary" size="default" @click="$router.push({ name: 'question.all_question' })" class="mt-2">
        所有考题
      </el-button>
      <el-button type="primary" size="default" @click="$router.push({ name: 'question.create' })" class="mt-2">
        我要出题
      </el-button>
    </section>
      ...
      <div class="border py-3 px-5 mt-3 bg-white" v-for="item of collection.data" :key="item.id">
        <div class="border-b py-3 mb-3 border-b-gray-200">
          ...
          <div class="mt-2 flex gap-2 items-stretch">
            ...
            <div class="text-sm bg-gray-50 rounded-sm px-3 flex items-center text-gray-500 border border-gray-200">
