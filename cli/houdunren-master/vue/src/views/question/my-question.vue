  ...
  <Layout>
    ...
    <section v-else>
        ...
        <div class="mt-2">
          <el-button-group>
            ...
            <el-button
              ...
              v-if="util.authorize(item.user?.id)">
              修改
            </el-button>
            <el-button type="danger" size="small" @click="del(item.id)" v-if="util.authorize(item.user?.id)">
              删除
            </el-button>
          </el-button-group>
        </div>
      ...
      <Pagination
        ...
        @current-change="$router.push({ name: 'question.user.list', query: { page: $event } })" />
    </section>
  </Layout>
</template>
