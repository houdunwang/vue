...
<template>
  <main v-if="collection">
    <div class="bg-white rounded-xl shadow-sm border">
      <section>
        <h1 class="text-center xl:py-8 text-2xl xl:text-4xl font-bold py-5">系统课程</h1>
        <div class="text-center xl:text-lg mx-5 mb-3">
          系统课程指从零开始学习一门编程语言，比如从零开始学习Javascript编程语言。
        </div>
      </section>
      <section class="grid grid-flow-row md:grid-cols-3 gap-5 p-5">
        <SystemItem v-for="item of collection" :key="item.id" :item="item" class="" />
      </section>
    </div>
  </main>
</template>
