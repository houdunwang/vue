  ...
  <HdCard>
    ...
    <div class="flex gap-2 flex-wrap items-center" v-if="signUsers?.data.length">
      <router-link
        ...
        <el-image :src="user.avatar" fit="cover" :lazy="true" class="w-16 h-16 cursor-pointer flex-shrink-0" />
        <span class="text-xs px-2 pb-2 truncate w-16">{{ user.name }}{{ user.name }}</span>
      </router-link>
    </div>
    ...
    <template #footer>
      <div class="text-center text-gray-600">
        ...
        <router-link :to="{ name: 'user.info' }" class="text-[#16a085]">收货地址</router-link>
        和
