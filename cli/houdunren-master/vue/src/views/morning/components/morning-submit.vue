  ...
  <main>
    ...
    <el-dialog v-model="dialogState" width="28%" append-to-body>
      <div class="flex flex-col justify-center items-center" v-if="user">
        <HdUserAvatar :user="user" class="rounded-lg w-72 h-72" />
        <span class="text-lg px-2 pb-2 mt-3">
          <div>
            恭喜
            <router-link
              :to="{ name: 'sign.space', params: { uid: user.id } }"
              target="_blank"
        ...
        <div class="pb-3" v-if="model.id">
          ...
          <div>
            <div class="flex flex-col gap-2 py-3 justify-start items-start">
              这次给早起朋友送的礼品：
              <a :href="model.url" target="_blank" class="text-green-600 inline-block w-72 h-72 overflow-hidden">
    ...
    <HdCard class="mt-3 leading-7">
      <div class="flex flex-col items-start gap-2">
        ...
        <div>
          已经设置了真实的 <router-link :to="{ name: 'user.info' }" class="text-[#16a085]">收货地址</router-link>
          和
          <router-link :to="{ name: 'user.mobile' }" class="text-[#16a085]">手机号</router-link>
          否则无法邮寄
        </div>
        ...
        <div class="flex flex-col items-start gap-2 mt-3">
          ...
          视频会放在平台，所以注意隐私信息<br />
          视频发微信：houdunren2021 或邮箱: 2300071698@qq.com <br />
          再次表示感谢，辛苦了兄弟姐妹
        </div>
