...
const { collection: learnVideos, lessonVideo } = useLearnHistory()
...
<template>
  <main>
            ...
            <a :href="href" class="flex py-3 border-b text-base hover:text-[#3b907f] cursor-pointer">
              <div class="flex items-baseline opacity-80">
                <div class="flex items-center mr-2">
                  <icon-play theme="outline" class="mr-2 text-[#16a085]" />
                  {{ v.title }}
                </div>
    ...
    <div v-else class="bg-white border text-gray-500 rounded-sm">课程正在录制中...</div>
  </main>
</template>
